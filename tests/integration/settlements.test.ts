...
async function closeOnePairResidence(
  ...
  await member.agent.post(`/residences/${code}/expenses`).send({
    name: 'Mercado',
    valueInCents: memberOwesInCents * 2,
...
describe('não-membro recebe 404 nas 6 rotas de acerto (RN-080)', () => {
  it('todas as rotas devolvem 404 para quem não é membro da residência', async () => {
    const { owner, code, settlementId } = await closeOnePairResidence(
      'Dono 404',
      'Membro 404',
      5000,
    );
    ...
    const routes: Array<[method: 'get' | 'post', path: string, body?: Record<string, unknown>]> = [
      ...
      [
        'post',
        `/residences/${code}/closures/${period}/settlements/${settlementId}/receipts/qualquer/complete`,
      ],
      ['post', `/residences/${code}/closures/${period}/settlements/${settlementId}/confirm`],
      [
        'post',
        `/residences/${code}/closures/${period}/settlements/${settlementId}/waive`,
        { reason: 'Teste de dispensa' },
      ],
    ...
    for (const [method, path, body] of routes) {
      const response =
        method === 'get'
          ? await outsider.agent.get(path)
          : await outsider.agent.post(path).send(body ?? {});
...
describe('fluxo completo por HTTP: intenção -> upload -> complete -> confirm', () => {
  ...
  it('percorre as rotas reais e o GET de listagem reflete cada transição', async () => {
    const { owner, member, code, settlementId } = await closeOnePairResidence(
      'Dono Fluxo',
      'Membro Fluxo',
      5000,
    );
    ...
    const listResponse = await owner.agent.get(
      `/residences/${code}/closures/${period}/settlements`,
    );
    ...
    const urlResponse = await member.agent.get(
      `/residences/${code}/closures/${period}/receipts/${receiptId}/url`,
    );
...
describe('dispensa de acerto pelo owner (D-07/RN-082)', () => {
  ...
  it('só o owner dispensa; dispensa a linha inteira mesmo com paidAt já preenchido; nunca vira SETTLED; notifica os dois lados; quita o mês', async () => {
    ...
    fakeStorage.simulateUpload(
      intent.body.upload.fields.key,
      VALID_SIGNATURE_BYTES['image/png']!,
      'image/png',
    );
...
describe('storageEnabled=false (D-18): degradação graciosa', () => {
  it('intenção de upload e URL de leitura respondem 503, mas confirmação, listagem, fechamento e despesas continuam 200', async () => {
    ...
    const missingUrlResponse = await owner.agent.get(
      `/residences/${code}/closures/${period}/receipts/qualquer/url`,
    );
    ...
    const urlResponse = await owner.agent.get(
      `/residences/${code}/closures/${period}/receipts/${storedReceipt.id}/url`,
    );
    ...
    const expensesResponse = await owner.agent.get(
...
describe('SETTLEMENT_PENDING é uma notificação por pessoa, somando os pares (D-31/RN-083)', () => {
  it('devedor em 3 pares no mesmo fechamento recebe uma única notificação, somando os 3 valores', async () => {
    ...
    await y.agent
      ...
      .send({ name: 'Gasto Y', valueInCents: 450, category: 'ALIMENTACAO', isRecurring: false });
    await z.agent
      .post(`/residences/${code}/expenses`)
      .send({ name: 'Gasto Z', valueInCents: 400, category: 'ALIMENTACAO', isRecurring: false });
    await w.agent
      .post(`/residences/${code}/expenses`)
      .send({ name: 'Gasto W', valueInCents: 350, category: 'ALIMENTACAO', isRecurring: false });
